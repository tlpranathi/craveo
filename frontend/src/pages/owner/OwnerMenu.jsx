...
export default function OwnerMenu() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await API.put(`/owner/menu/${editingId}`, payload)
        setMenu((prev) => prev.map((m) => (m._id === editingId ? { ...m, ...payload } : m)))
      } else {
  ...
  if (loading) return <p className="text-gray-500 text-sm">Loading menu...</p>
    ...
    <div>
      <div className="flex justify-between items-center mb-5">
        <h2 className="text-lg font-semibold text-gray-900">
          Menu <span className="text-gray-400 font-normal text-sm ml-1">({menu.length} items)</span>
        </h2>
        <button
          onClick={openCreate}
            ...
            {menu.map((item) => (
              <tr key={item._id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right space-x-2 whitespace-nowrap">
                  <button onClick={() => openEdit(item)} className="text-gray-600 hover:underline font-medium">Edit</button>
                  <button onClick={() => handleDelete(item._id, item.name)} className="text-red-500 hover:underline font-medium">Delete</button>
                </td>
              </tr>
            ))}
            {menu.length === 0 && (
              <tr>
                <td colSpan={4} className="px-4 py-8 text-center text-gray-400">No menu items yet.</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <input type="number" placeholder="Price" required min="0" step="0.01" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-craveo-400"/>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 border border-gray-300 text-gray-700 py-2.5 rounded-lg font-medium hover:bg-gray-50 transition">Cancel</button>
